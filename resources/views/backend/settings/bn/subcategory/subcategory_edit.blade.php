        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="box box-info">
                    <!-- form start -->
                    <form action="{{ route('bn-subcategories.update', $subcategory->subcat_id) }}" method="post" class="form-horizontal col-sm-offset-1">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <div class="box-body">
                                ...
                                <div class="col-sm-6">
                                    <select class="form-control col-sm-6" name="category" id="category">
                                        @foreach($categories as $category)
                                            <option value="{{ $category->cat_id }}" {{ $category->cat_id == $subcategory->cat_id ? 'selected' : '' }}>{{ $category->cat_name_bn }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
