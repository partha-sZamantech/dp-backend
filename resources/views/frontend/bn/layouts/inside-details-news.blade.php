<script>
    let insideMoreNewses = @json($insideMoreNews);


    let descs = document.getElementsByClassName('firstDescription');
    // for(let i = 0; i< desc.length; i++){

        let descParam = descs[0].querySelectorAll('p');

        let insertRelatedNewses = (title, src, href) => {

            let relatedNews = document.createElement(`div`);
            relatedNews.className = `inside-news marginTop20 marginBottom20 d-print-none`;
        ...
        let googleNewses = () => {
            let link = document.createElement(`a`);
            link.className = `text-center marginTop10 marginBottom10 d-print-none`;
        ...
        descParam.forEach((item, i) => {
            ...
            if (i > 0 && i % 3 === 0 && insideMoreNewses[itemIncrement]) {
                descParam[0].parentNode.insertBefore(insertRelatedNewses(insideMoreNewses[itemIncrement].content_heading, fJsNewsImgPaths(insideMoreNewses[itemIncrement].img_xs_path), fJsNewsURLs(insideMoreNewses[itemIncrement].content_id, insideMoreNewses[itemIncrement].category.cat_slug, insideMoreNewses[itemIncrement].subcategory?.subcat_slug)), descParam[i-1].nextSibling);
        ...
        function fJsNewsURLs(content_id, cat_slug, subcat_slug='') {
