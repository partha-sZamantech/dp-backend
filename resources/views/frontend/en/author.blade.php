...
@section('title', $author->author_name . ' । Dhaka Prokash')

@section('customMeta')
    <link rel="canonical" href="{{fEnRoot('author/'.$author->author_slug)}}">
    <meta name="description" content="{{!empty($author->author_bio) ? $author->author_bio : ''}}"/>

    <meta property="og:type" content="website" />
    <meta property="og:url" content="{{fEnRoot('author/'.$author->author_slug)}}"/>
    <meta property="og:title" content="{{$author->author_name. ' । Dhaka Prokash'}}"/>
    <meta property="og:image" content="{{ asset(config('appconfig.commonImagePath').Cache::get('enSiteSettings')->og_image) }}"/>
    <meta property="og:description" content="{{!empty($author->author_bio) ? $author->author_bio : ''}}"/>

    <meta name="twitter:title" content="{{$author->author_name. ' । Dhaka Prokash'}}">
    <meta name="twitter:description" content="{{!empty($author->author_bio) ? $author->author_bio : ''}}">
    ...
    <div class="main-content">
        <div class="container">
            ...
            <p class="breadcrumb">
                ...
                <span>&raquo;</span>
                <a href="{{ url('topic/'.$author->author_slug) }}" class="active">{{ $author->author_name }}</a>
            </p>
                ...
                <div class="col-sm-12">
                    <div class="author-box marginBottom20" style="display: flex; align-items: center">
                        @if(!empty($author->img_path))
                            <img src="{{ asset(config('appconfig.authorImagePath').$author->img_path) }}" style="width: 80px ; height: 80px ; border-radius: 50%; margin-bottom: 0; overflow: hidden; object-fit: cover;" alt="{{ $author->author_name }}">
                        @else
                            <img src="{{asset(config('appconfig.commonImagePath').'favicon.png')}}" alt="Dhaka Prokash">
                        @endif
                        <div>
                            <p style="font-size: 32px; margin-left: 10px;">{{ $author->author_name }}</p>
                            <p style="font-size: 16px; margin-left: 10px;">{!! $author->author_bio !!}</p>
                        </div>
                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-9">
                                        <small class="text-muted">
                                            <a href="{{ fEnRoot($content->category->cat_slug) }}" class="jC_tag"> {{ $content->category->cat_name }} </a>
