...
@if($detailsRightThreeAd && $hasDetailsRightThreeAdTime)
    ...
    <div class="marginBottom20 {{ $detailsRightThreeAd->type != 4 ? 'advertisement' : '' }}">
        <div class="hidden-sm hidden-xs {{ $detailsRightThreeAd->type != 4 ? 'ad-box' : '' }}">
        ...
        <div class="hidden-md hidden-lg {{ $detailsRightThreeAd->type != 4 ? 'ad-box' : '' }}">
            {{-- Type 1=DFP, 2=Code, 3=Image --}}
            @if($detailsRightThreeAd->type == 3)
                <a href="{{ $detailsRightThreeAd->external_link }}" target="_blank" rel="nofollow">
