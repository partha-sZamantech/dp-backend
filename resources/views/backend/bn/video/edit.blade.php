...
@section('title', 'BN Video Edit')

@section('content')
  <!-- Content Header (Page header) -->
  <section class="content-header">
    <ol class="breadcrumb">
      <li><a href="{{ route('bn-videos.index') }}">BN Video List</a></li>
      <li class="active">Video edit</li>
    </ol>
  </section>
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-info">
          ...
          <!-- /.box-header -->
          <!-- form start -->
          <form action="{{ route('bn-videos.update', $video->id) }}" method="post" enctype="multipart/form-data" class="form-horizontal col-sm-offset-1">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div class="box-body">
                    ...
                    <div class="col-sm-6">
                        <select name="category" id="category" class="form-control" required>
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}"{{ $video->cat_id == $category->id ? ' selected' : '' }}>{{ $category->name_bn }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
              ...
              {{--<div class="form-group" id="link">
                ...
                <div class="col-sm-6">
                  <input type="text" name="link" class="form-control" id="link" placeholder="Link" value="{{ $video->link }}">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <select name="target" id="target" class="form-control" required>
                            <option value="1"{{ $video->target == 1 ? ' selected' : '' }}>Same Site</option>
                            <option value="2"{{ $video->target == 2 ? ' selected' : '' }}>New Window</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <label class="radio-inline">
                            <input type="radio" name="is_live" value="1" {{ $video->is_live == 1 ? 'checked' : '' }}>Yes
                        </label>
                        <label class="radio-inline">
                            <input type="radio" name="is_live" value="0" {{ $video->is_live == 0 ? 'checked' : '' }}>No
                        </label>
                    </div>
                </div>
