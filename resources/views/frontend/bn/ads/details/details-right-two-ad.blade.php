...
@if($detailsRightTwoAd && $hasDetailsRightTwoAdTime)
    ...
    <div class="marginBottom20 {{ $detailsRightTwoAd->type != 4 ? 'advertisement' : '' }}">
        <div class="hidden-sm hidden-xs {{ $detailsRightTwoAd->type != 4 ? 'ad-box' : '' }}">
        ...
        <div class="hidden-md hidden-lg {{ $detailsRightTwoAd->type != 4 ? 'ad-box' : '' }}">
            {{-- Type 1=DFP, 2=Code, 3=Image --}}
            @if($detailsRightTwoAd->type == 3)
                <a href="{{ $detailsRightTwoAd->external_link }}" target="_blank" rel="nofollow">
